...
export default function Album() {
  ...
  useEffect(() => {
    ...
    fetch('https://topacash.onrender.com/clients')
      .then((response) => {
        if (!response.ok) {
  ...
  const handleViewLoans = (client) => {
    ...
    fetch(`https://topacash.onrender.com/loans`)
      .then((response) => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then((data) => {
        // console.log(data)
        // Filter loans based on the selected client's ID
  ...
  const handleLoanPaid = (loanId) => {
    // Send a DELETE request to delete the loan
